...
<a href="#open-comic-library" id="open-comic-library" >
	<div id="comicker-comic-area" class="no-image">
		...
		<img id="comic-page" class="comic-page <?php if( !$have_img ) { echo 'hidden'; } ?>" src="<?php if( $have_img ) { echo $img_src[0]; } ?>">

		<input type="text" name="_comic_id" class="comic-id hidden tags-list" type="hidden" value="<?php echo esc_attr( $img_id ); ?>">
		<textarea class="hidden"></textarea>
		<?php wp_nonce_field( 'save_comic', 'comic_editor_nonce' ); ?>
	</div>
</a>

<button class="remove-comic button hidden">Remove Comic</button>